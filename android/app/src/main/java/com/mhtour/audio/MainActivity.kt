...
class MainActivity : ComponentActivity() {
    ...
    private fun home() {
        // UI HOME ONLY: server, LiveKit, and session flow are unchanged.
        ...
        val scrollContent = LinearLayout(this).apply {
            ...
            setPadding(dp(22), dp(8), dp(22), dp(24))
        }

        // 3D logo taken from the supplied MH Tour reference design.
        val logo3d = ImageView(this).apply {
            setImageResource(R.drawable.mh_tour_logo_3d)
            scaleType = ImageView.ScaleType.FIT_CENTER
            contentDescription = "Logo 3D MH Tour"
        }
        scrollContent.addView(logo3d, LinearLayout.LayoutParams(-1, dp(250)))
        addGap(scrollContent, 4)

        // Elegant white title card matching the supplied home-screen reference.
        val titleCard = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            gravity = Gravity.CENTER
            setPadding(dp(16), dp(12), dp(16), dp(12))
            background = bg(Color.rgb(255, 255, 255), 22f, Color.rgb(226, 235, 231), 1)
            elevation = dp(2).toFloat()
        }
        val title = tv("MH Tour", 25f, Color.rgb(7, 103, 91), true)
        title.gravity = Gravity.CENTER
        titleCard.addView(title, LinearLayout.LayoutParams(-1, dp(38)))

        val divider = LinearLayout(this).apply {
            gravity = Gravity.CENTER
        }
        val lineLeft = View(this).apply { setBackgroundColor(Color.rgb(198, 145, 36)) }
        val ornament = tv("✦", 15f, Color.rgb(198, 145, 36), true)
        ornament.gravity = Gravity.CENTER
        val lineRight = View(this).apply { setBackgroundColor(Color.rgb(198, 145, 36)) }
        divider.addView(lineLeft, LinearLayout.LayoutParams(0, dp(1), 1f))
        divider.addView(ornament, LinearLayout.LayoutParams(dp(34), dp(24)))
        divider.addView(lineRight, LinearLayout.LayoutParams(0, dp(1), 1f))
        titleCard.addView(divider, LinearLayout.LayoutParams(-1, dp(24)))

        val subtitle = tv("Aplikasi Pemandu Suara\nUntuk Jemaah Haji & Umroh", 17f, Color.rgb(12, 104, 94), true)
        subtitle.gravity = Gravity.CENTER
        subtitle.setLineSpacing(0f, 1.08f)
        titleCard.addView(subtitle, LinearLayout.LayoutParams(-1, dp(64)))
        scrollContent.addView(titleCard, LinearLayout.LayoutParams(-1, dp(138)))
        addGap(scrollContent, 30)

        // Primary action — capitalization follows the supplied reference.
        val jamaahBtn = primaryButton("🎧  Dengarkan Sebagai Jemaah", Color.rgb(0, 126, 115)).apply {
            textSize = 16.5f
            minHeight = dp(60)
            background = bg(Color.rgb(0, 126, 115), 32f, Color.rgb(198, 145, 36), 1)
            elevation = dp(2).toFloat()
            contentDescription = "Dengarkan Sebagai Jemaah"
        }
        scrollContent.addView(jamaahBtn, LinearLayout.LayoutParams(-1, dp(60)))
        addGap(scrollContent, 14)

        // Guide action renamed exactly as requested.
        val guideBtn = outlineButton("🎙  Lanjut Sebagai Pemandu").apply {
            textSize = 16.5f
            minHeight = dp(60)
            background = bg(Color.WHITE, 32f, Color.rgb(31, 116, 106), 2)
            contentDescription = "Lanjut Sebagai Pemandu"
        }
        scrollContent.addView(guideBtn, LinearLayout.LayoutParams(-1, dp(60)))
        addGap(scrollContent, 28)

        // Donation action opens the in-app donation information page.
        val donation = primaryButton("♡  Donasi Seikhlasnya", Color.rgb(0, 111, 101)).apply {
            textSize = 15f
            minHeight = dp(52)
            background = bg(Color.rgb(0, 111, 101), 28f, Color.rgb(198, 145, 36), 1)
            contentDescription = "Donasi Seikhlasnya"
            setOnClickListener { donationScreen() }
        }
        scrollContent.addView(donation, LinearLayout.LayoutParams(dp(270), dp(52)).apply {
            gravity = Gravity.CENTER_HORIZONTAL
        })
        addGap(scrollContent, 16)

        val managed = tv("Suara di Setiap Langkah Ibadah", 11f, Color.rgb(100, 116, 139), false)
        managed.gravity = Gravity.CENTER
        scrollContent.addView(managed, LinearLayout.LayoutParams(-1, dp(28)))
    ...
    private fun donationScreen() {
        ...
        val root = page(
            "Donasi Seikhlasnya",
            "Dukung pengembangan MH Tour sesuai kemampuan Anda."
        )
        val body = contentColumn()

        val qrCard = card().apply {
            gravity = Gravity.CENTER_HORIZONTAL
        }

        qrCard.addView(
            tv("Scan QR untuk berdonasi", 18f, green, true),
        ...
        val qr = ImageView(this).apply {
            ...
            scaleType = ImageView.ScaleType.FIT_CENTER
            contentDescription = "QR Code Donasi Seikhlasnya"
            setBackgroundColor(Color.WHITE)
            setPadding(dp(4), dp(4), dp(4), dp(4))
        }
        qrCard.addView(qr, LinearLayout.LayoutParams(-1, dp(330)).apply {
            gravity = Gravity.CENTER_HORIZONTAL
        })

        addGap(qrCard, 10)
        val urlLabel = tv("Link Donasi", 13f, muted, true)
        qrCard.addView(urlLabel)
        ...
        val urlText = TextView(this).apply {
            ...
            background = bg(Color.rgb(248, 250, 252), 12f, Color.rgb(226, 232, 240), 1)
            autoLinkMask = android.text.util.Linkify.WEB_URLS
            linksClickable = true
            movementMethod = android.text.method.LinkMovementMethod.getInstance()
        }
        qrCard.addView(urlText, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT))

        body.addView(qrCard, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT))
        addGap(body, 14)

        val download = primaryButton("⬇  Unduh QR Code").apply {
            contentDescription = "Unduh QR Code Donasi"
            setOnClickListener { downloadDonationQr() }
        }
        body.addView(download, LinearLayout.LayoutParams(-1, dp(54)))

        addGap(body, 10)
        val open = outlineButton("Buka Link Donasi").apply {
            contentDescription = "Buka Link Donasi"
            setOnClickListener {
                try {
                    ...
                    Toast.makeText(
                        this@MainActivity,
                        "Tidak dapat membuka link donasi.",
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
        }
        body.addView(open, LinearLayout.LayoutParams(-1, dp(52)))

        addGap(body, 10)
        val back = outlineButton("← Kembali").apply {
            contentDescription = "Kembali ke Beranda"
            setOnClickListener { home() }
        }
        body.addView(back, LinearLayout.LayoutParams(-1, dp(52)))
    ...
    private fun donationQrBitmap(): Bitmap {
        return BitmapFactory.decodeResource(resources, R.drawable.donation_qr)
            ?: throw IllegalStateException("QR donasi tidak tersedia")
    }

    private fun downloadDonationQr() {
        try {
            val bitmap = donationQrBitmap()
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                val values = ContentValues().apply {
                    put(MediaStore.Images.Media.DISPLAY_NAME, "MH-Tour-QR-Donasi.png")
                    put(MediaStore.Images.Media.MIME_TYPE, "image/png")
                    put(
                        MediaStore.Images.Media.RELATIVE_PATH,
                        android.os.Environment.DIRECTORY_DOWNLOADS + "/MH Tour"
                    )
                    put(MediaStore.Images.Media.IS_PENDING, 1)
                }
                val resolver = contentResolver
                val uri = resolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values)
                    ?: throw IllegalStateException("Gagal membuat file QR")
                try {
                    resolver.openOutputStream(uri)?.use { output ->
                        if (!bitmap.compress(Bitmap.CompressFormat.PNG, 100, output)) {
                            throw IllegalStateException("Gagal menulis QR")
                        }
                    } ?: throw IllegalStateException("Gagal membuka file QR")
                    values.clear()
                    values.put(MediaStore.Images.Media.IS_PENDING, 0)
                    resolver.update(uri, values, null, null)
                    Toast.makeText(
                        this,
                        "QR Code berhasil disimpan di Unduhan/MH Tour.",
                        Toast.LENGTH_LONG
                    ).show()
                } catch (e: Exception) {
                ...
                val intent = Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
                    ...
                    type = "image/png"
                    putExtra(Intent.EXTRA_TITLE, "MH-Tour-QR-Donasi.png")
                }
                pendingDonationQr = bitmap
                startActivityForResult(intent, REQUEST_SAVE_DONATION_QR)
            }
        } catch (e: Exception) {
            Toast.makeText(this, "QR Code belum dapat disimpan.", Toast.LENGTH_LONG).show()
        }
    }

    private var pendingDonationQr: Bitmap? = null

    private val REQUEST_SAVE_DONATION_QR = 2401

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        if (requestCode != REQUEST_SAVE_DONATION_QR) return

        val bitmap = pendingDonationQr
        pendingDonationQr = null
        if (resultCode != RESULT_OK || data?.data == null || bitmap == null) return
    ...
    private fun startQrScanner() {
        IntentIntegrator(this).apply {
            ...
            setBeepEnabled(true)
            // Lock the embedded ZXing camera activity to the app's portrait orientation.
            // MainActivity is already portrait; this prevents the QR scanner from opening landscape.
